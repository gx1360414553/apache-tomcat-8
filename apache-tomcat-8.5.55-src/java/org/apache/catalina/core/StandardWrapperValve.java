...
final class StandardWrapperValve
    ...
    public final void invoke(Request request, Response response)
        ...
        StandardWrapper wrapper = (StandardWrapper) getContainer(); //StandardWrapper和 Value对应
        ...
        //Servlet默认是在第一次请求的时候实例化。延迟加载  因为loadup默认设置为 -1
        try {
            if (!unavailable) {
        ...
        MessageBytes requestPathMB = request.getRequestPathMB(); // 获取path
        ...
        try {
            ...
            try {
                if (servlet != null) {
                    wrapper.deallocate(servlet); //回收到Servlet实例池中
                }
            } catch (Throwable e) {
                ExceptionUtils.handleThrowable(e);
